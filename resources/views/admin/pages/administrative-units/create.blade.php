        ...
        <div class="col-12">
            <div class="card mb-4 mx-4">
                <div class="card-header pb-0 px-3">
                    <h5 class="mb-0">Tạo đơn vị hành chính mới</h5>
                </div>
                <div class="card-body pt-4 p-3">
                    @include('admin.pages.components.success-error')

                    <form action="{{ route('admin.administrative-units.store') }}" method="POST">
                        @csrf
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="code" id="code" class="form-control @error('code') is-invalid @enderror"
                                    value="{{ old('code') }}" placeholder="VD: VTE-PROV, VGV-DIST" maxlength="10" required>
                                @error('code')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror"
                                    value="{{ old('name') }}" placeholder="VD: Vientiane Province" required>
                                @error('name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="local_name" id="local_name" class="form-control @error('local_name') is-invalid @enderror"
                                    value="{{ old('local_name') }}" placeholder="VD: ແຂວງວຽງຈັນ">
                                @error('local_name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="level" id="level" class="form-control @error('level') is-invalid @enderror" required>
                                    ...
                                    <option value="1" {{ old('level') == '1' ? 'selected' : '' }}>Cấp 1 (Tỉnh/Thành phố)</option>
                                    <option value="2" {{ old('level') == '2' ? 'selected' : '' }}>Cấp 2 (Huyện/Quận)</option>
                                    <option value="3" {{ old('level') == '3' ? 'selected' : '' }}>Cấp 3 (Xã/Phường)</option>
                                </select>
                                @error('level')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
